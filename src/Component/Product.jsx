...
const Product = ({data,handleDelete}) => {
  const {brandName, image, name, email, price, productType,shortDescription, rating,_id} = data;
  ...
  const renderStars = (rating) => {
    ...
    for (let i = 1; i <= maxRating; i++) {
      if (i <= rating) {
        ...
        stars.push(<i key={i} className='mdi mdi-star-outline text-yellow-400'></i>);
      }
    }
    return stars;
  };
        ...
        <div className=" md:w-1/2 px-10">
          ...
          <div>
            <div className="inline-block align-bottom mr-5">
              ...
              <p className="text-2xl leading-none align-baseline m-3">{shortDescription}</p>
            </div>
            <div className='mt-4'>
              <p>Rating</p>
            ...
            <div className="block align-bottom">
              <button onClick={()=>handleDelete(_id)} className="bg-red-500 opacity-75 hover:opacity-100 text-white hover:text-gray-900 rounded-full px-10 py-2 font-semibold"><i className="mdi mdi-cart -ml-2 mr-2"></i> DELETE</button>
            </div>
          </div>
        </div>
      </div>
